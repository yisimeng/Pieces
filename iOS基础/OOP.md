...
封装主要是将数据结构和函数进行抽象组合到一起。
数据结构是来描述数据信息的。函数就是一个黑盒算法，有输入输出，执行逻辑。

封装将数据结构和函数捆绑，如果只是想使用对象的某一个函数，就必须依赖这个对象及对象的所有依赖。

如果是函数式编程，就没有这么重的依赖了。但是函数式编程，对于代码的可读性，以及模块的区分不明显。

面向对象是在架构设计是非常好的思想，但如果只是简单映射到程序实现上，还是会有些隐患。
